...
function tekouinlab_get_api_config() {
    global $CFG;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($flags)) {
        ...
        $apiConfig = tekouinlab_get_api_config();
        ...
        if ($apiConfig) {
            ...
            if ($curl->get_errno()) {
                echo $OUTPUT->notification(get_string('submitflagfailed', 'mod_tekouinlab'), 'error');
            } else {
                // Handle the response (optional)
                $response_data = json_decode($response);
                if (isset($response_data->status) && $response_data->status === 'success') {
                    echo $OUTPUT->notification(get_string('submitflagsuccess', 'mod_tekouinlab'), 'success');
                } else {
                    echo $OUTPUT->notification(get_string('submitflagfailed', 'mod_tekouinlab'), 'error');
                }
            }
        } else {
            echo $OUTPUT->notification(get_string('api_configuration_error', 'mod_tekouinlab'), 'error');
        }
    } else {
        echo $OUTPUT->notification(get_string('noflagsubmitted', 'mod_tekouinlab'), 'error');
    }
} else {
    // If the form wasn't submitted via POST, just redirect back to the lab view page
    redirect(new moodle_url('/mod/tekouinlab/view.php', array('id' => $cm->id)));
}
