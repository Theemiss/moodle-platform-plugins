...
function check_student_vm( $userid) {
    $apiurl = get_config('local_platformbridge', 'apiurl'); // Django API URL
...
function spawn_vm_session($userid) {
    ...
    if (empty($apiurl) || empty($apikey)) {
        return [
            'status' => 'error',
            'message' => get_string('api_config_error', 'mod_cloudvm'),
        ];
    }
    ...
    if ($curl->get_errno()) {
        return [
            'status' => 'error',
            'message' => get_string('lab_spawn_error', 'mod_cloudvm'),
        ];
    }
    ...
    if (isset($response_data['status']) && $response_data['status'] === 'running') {
        return [
            'status' => 'success',
            'message' => get_string('labspawned', 'mod_cloudvm'),
...
function destroy_vm_session($userid) {
    ...
    if ($curl->get_errno()) {
        return [
            'status' => 'error',
            'message' => get_string('lab_destroy_error', 'mod_cloudvm'),
        ];
    }
    ...
    if (isset($response_data['status']) && $response_data['status'] === 'success') {
        return [
            'status' => 'success',
            'message' => get_string('labdestroyed', 'mod_cloudvm'),
        ];
    }
